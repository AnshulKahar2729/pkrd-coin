...
const BuyerDashboard = () => {
  ...
  const [buyingBids, setBuyingBids] = useState([]);
  ...
  useEffect(() => {
    fetchBuyerBidsList();
  }, [filterData]); // Trigger fetchBuyerBidsList when filterData changes
  ...
  return (
    <div className="lg:h-screen flex flex-col lg:flex-row lg:pt-16 w-full">
      <div className="w-full lg:w-[20%]">
        <BuySellSidebar onLookFor={onLookFor} type="Buy" />
      </div>
      <div className="w-full lg:flex-1 p-4 overflow-hidden lg:min-h-screen overflow-y-scroll">
        <div className="mt-5 mb-5">
          <h2 className="text-[#29A645] text-4xl">Buy PKRD.</h2>
        </div>
        <div>
        ...
        <div className="border-[1px] border-gray-300 rounded-md">
          <table className="lg:mr-20 rounded-tl-md rounded-tr-md overflow-hidden w-full place">
            <tr className="py-4 px-4 lg:grid grid-cols-4 bg-[#29A645] hidden text-white ">
              <th className="flex items-center text-left ">Buy from</th>
              <th className="flex items-center">Pay with</th>
              <th className="flex items-center text-left ">Avg. trade speed</th>
            ...
            ) : buyingBids?.data?.length === 0 ? (
              <div className="flex justify-center items-center h-24">
                <p className="text-2xl text-gray-500">No bids available</p>
              </div>
            ) : (
              buyingBids?.data?.map((bid, index) => (
                <BuyerCard key={index} bid={bid} />
              ))
            )}
          </table>
        </div>

        {loading ? null : error ? null : buyingBids?.data?.length === 0 ? (
          <div className="flex justify-center items-center h-24">
            <p className="text-2xl text-gray-500">No bids available</p>
          </div>
        ) : (
